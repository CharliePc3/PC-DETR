...
VARIANT="${VARIANT:?Set VARIANT to core, spatial, or spatial_antialias}"
MAX_USED_MIB="${MAX_USED_MIB:-4000}"
POLL_SECONDS="${POLL_SECONDS:-60}"

case "${VARIANT}" in
  core)
    OUTPUT_TAG="scalefirst_core_rank64"
    EXTRA_ARGS=(
    ...
    ;;
  spatial)
    OUTPUT_TAG="scalefirst_spatial_rank64"
    EXTRA_ARGS=(
    ...
    ;;
  spatial_antialias)
    OUTPUT_TAG="scalefirst_spatial_antialias_rank64"
    EXTRA_ARGS=(
      --sdsr-use-local-reassembly
      --sdsr-use-directional-guide
...
OUTPUT_NAME="coco_medium_tb16_bs8_g6_p3p4p5_ms_exp_cdn_sdsrv3_${OUTPUT_TAG}_seed42_e24"
OUTPUT_DIR="${ROOT}/output/${OUTPUT_NAME}"
TRAIN_LOG="${ROOT}/${OUTPUT_NAME}_nohup.log"
RUN_LOG="${ROOT}/sdsr_v3_${VARIANT}_gpu${GPU_ID}_runner_nohup.log"
PID_FILE="${ROOT}/sdsr_v3_${VARIANT}_gpu${GPU_ID}.pid"
LOCK_FILE="${ROOT}/sdsr_v3_${VARIANT}_gpu${GPU_ID}.lock"
RESULT_DIR="${ROOT}/output/sdsr_v3_medium_ablation"
RESULT_TSV="${RESULT_DIR}/${VARIANT}_seed42.tsv"

cd "${ROOT}"
mkdir -p "${RESULT_DIR}" "/tmp/matplotlib_sdsr_v3_${VARIANT}_${GPU_ID}"
export CUDA_VISIBLE_DEVICES="${GPU_ID}"
export PYTHONPATH="${ROOT}/src"
export MPLCONFIGDIR="/tmp/matplotlib_sdsr_v3_${VARIANT}_${GPU_ID}"
