...
VARIANT="${VARIANT:?Set VARIANT to csp_core, csp_rank96, csp_bidir, or csp_bidir_antialias}"
...
case "${VARIANT}" in
  csp_core)
    OUTPUT_TAG="csp_rank64_stableinit${DETECTOR_INIT_SEED}"
    PROJECTOR_ARGS=(
      --projector-type sdsr_v8
    ...
    ;;
  csp_rank96)
    OUTPUT_TAG="csp_rank96_stableinit${DETECTOR_INIT_SEED}"
    PROJECTOR_ARGS=(
      --projector-type sdsr_v8
    ...
    ;;
  csp_bidir)
    OUTPUT_TAG="csp_bidir32_rank64_stableinit${DETECTOR_INIT_SEED}"
    PROJECTOR_ARGS=(
      --projector-type sdsr_v8
      --sdsr-rank-channels 64
    ...
    ;;
  csp_bidir_antialias)
    OUTPUT_TAG="csp_bidir32_antialias_rank64_stableinit${DETECTOR_INIT_SEED}"
    PROJECTOR_ARGS=(
      --projector-type sdsr_v8
      --sdsr-rank-channels 64
      --sdsr-cross-scale-mode bidirectional
...
OUTPUT_NAME="coco_medium_tb16_bs8_g6_p3p4p5_ms_exp_cdn_sdsrv8_${OUTPUT_TAG}_seed42_e24"
OUTPUT_DIR="${ROOT}/output/${OUTPUT_NAME}"
TRAIN_LOG="${ROOT}/${OUTPUT_NAME}_nohup.log"
RUN_LOG="${ROOT}/sdsr_v8_${VARIANT}_gpu${GPU_ID}_runner_nohup.log"
PID_FILE="${ROOT}/sdsr_v8_${VARIANT}_gpu${GPU_ID}.pid"
LOCK_FILE="${ROOT}/sdsr_v8_${VARIANT}_gpu${GPU_ID}.lock"
RESULT_DIR="${ROOT}/output/sdsr_v8_medium_ablation"
RESULT_TSV="${RESULT_DIR}/${VARIANT}_seed42.tsv"

cd "${ROOT}"
mkdir -p "${RESULT_DIR}" "/tmp/matplotlib_sdsr_v8_${VARIANT}_${GPU_ID}"
export CUDA_VISIBLE_DEVICES="${GPU_ID}"
export PYTHONPATH="${ROOT}/src"
export MPLCONFIGDIR="/tmp/matplotlib_sdsr_v8_${VARIANT}_${GPU_ID}"
