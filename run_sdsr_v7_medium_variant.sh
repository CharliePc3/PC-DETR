...
VARIANT="${VARIANT:?Set VARIANT to stable_b0, scale_first, scale_first_rank96, scale_first_topdown, scale_first_bidir, or scale_first_bidir_antialias}"
...
case "${VARIANT}" in
  stable_b0)
    OUTPUT_TAG="b0_stableinit${DETECTOR_INIT_SEED}"
    PROJECTOR_ARGS=(
      --projector-type multiscale
    )
    ;;
  scale_first)
    OUTPUT_TAG="scalefirst_rank64_stableinit${DETECTOR_INIT_SEED}"
    PROJECTOR_ARGS=(
      --projector-type sdsr_v7
    ...
    ;;
  scale_first_rank96)
    OUTPUT_TAG="scalefirst_rank96_stableinit${DETECTOR_INIT_SEED}"
    PROJECTOR_ARGS=(
      --projector-type sdsr_v7
    ...
    ;;
  scale_first_topdown)
    OUTPUT_TAG="scalefirst_topdown32_rank64_stableinit${DETECTOR_INIT_SEED}"
    PROJECTOR_ARGS=(
      --projector-type sdsr_v7
      --sdsr-rank-channels 64
      --sdsr-cross-scale-mode topdown
      --sdsr-cross-scale-rank 32
      --no-sdsr-use-local-reassembly
      --no-sdsr-use-directional-guide
      --no-sdsr-use-phase-downsample
    ...
    ;;
  scale_first_bidir)
    OUTPUT_TAG="scalefirst_bidir32_rank64_stableinit${DETECTOR_INIT_SEED}"
    PROJECTOR_ARGS=(
      --projector-type sdsr_v7
      --sdsr-rank-channels 64
    ...
    ;;
  scale_first_bidir_antialias)
    OUTPUT_TAG="scalefirst_bidir32_antialias_rank64_stableinit${DETECTOR_INIT_SEED}"
    PROJECTOR_ARGS=(
      --projector-type sdsr_v7
      --sdsr-rank-channels 64
      --sdsr-cross-scale-mode bidirectional
...
OUTPUT_NAME="coco_medium_tb16_bs8_g6_p3p4p5_ms_exp_cdn_sdsrv7_${OUTPUT_TAG}_seed42_e24"
OUTPUT_DIR="${ROOT}/output/${OUTPUT_NAME}"
TRAIN_LOG="${ROOT}/${OUTPUT_NAME}_nohup.log"
RUN_LOG="${ROOT}/sdsr_v7_${VARIANT}_gpu${GPU_ID}_runner_nohup.log"
PID_FILE="${ROOT}/sdsr_v7_${VARIANT}_gpu${GPU_ID}.pid"
LOCK_FILE="${ROOT}/sdsr_v7_${VARIANT}_gpu${GPU_ID}.lock"
RESULT_DIR="${ROOT}/output/sdsr_v7_medium_ablation"
RESULT_TSV="${RESULT_DIR}/${VARIANT}_seed42.tsv"

cd "${ROOT}"
mkdir -p "${RESULT_DIR}" "/tmp/matplotlib_sdsr_v7_${VARIANT}_${GPU_ID}"
export CUDA_VISIBLE_DEVICES="${GPU_ID}"
export PYTHONPATH="${ROOT}/src"
export MPLCONFIGDIR="/tmp/matplotlib_sdsr_v7_${VARIANT}_${GPU_ID}"
