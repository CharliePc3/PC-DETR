...
QUEUE_LOG="${ROOT}/sdsr_medium_ablation_gpu${GPU_ID}_queue_nohup.log"
PID_FILE="${ROOT}/sdsr_medium_ablation_gpu${GPU_ID}_queue.pid"
LOCK_FILE="${ROOT}/sdsr_medium_ablation_gpu${GPU_ID}_queue.lock"
SUMMARY_DIR="${ROOT}/output/sdsr_medium_ablation"
SUMMARY_TSV="${SUMMARY_DIR}/summary.tsv"

cd "${ROOT}"
mkdir -p "${SUMMARY_DIR}" /tmp/matplotlib_sdsr_medium
export CUDA_VISIBLE_DEVICES="${GPU_ID}"
export PYTHONPATH="${ROOT}/src"
export MPLCONFIGDIR="/tmp/matplotlib_sdsr_medium"
export ALBUMENTATIONS_DISABLE_VERSION_CHECK=1

exec 9>"${LOCK_FILE}"
if ! flock -n 9; then
  printf '[%s] Another SDSR medium queue already holds %s\n' "$(date '+%F %T')" "${LOCK_FILE}" >&2
  exit 1
fi
printf '%s\n' "$$" > "${PID_FILE}"
...
run_variant() {
  ...
  shift 2
  extra_args=("$@")
  output_name="coco_medium_tb16_bs8_g6_p3p4p5_ms_exp_cdn_sdsr_${output_tag}_seed42_e24"
...
log_status "QUEUE START on physical GPU ${GPU_ID}, commit=$(git rev-parse --short HEAD)"

run_variant b0_multiscale b0_multiscale \
  --projector-type multiscale

run_variant s0_router s0_router \
  --projector-type sdsr \
...
run_variant s1_semantic_local s1_semantic_local \
  --projector-type sdsr \
  --sdsr-detail-channels 32 \
...
run_variant s2_directional s2_directional \
  --projector-type sdsr \
  --sdsr-detail-channels 32 \
  --sdsr-use-local-reassembly \
