...
"""Two-basis P5 reassembly with a frequency-aware detail initialization."""
...
class HighPassTwoBasisDownsample(nn.Module):
    """Preserve a low-pass response and initialize the detail basis as high-pass."""
    ...
    def __init__(self, channels: int, anti_alias: bool):
        ...
        # delta - binomial blur starts as a zero-DC edge/detail extractor. The
        # kernel remains learnable, so training can adapt it per channel.
        high_pass = -blur_kernel
        high_pass[1, 1] += 1.0
        with torch.no_grad():
            self.detail.weight.copy_(
                high_pass[None, None].expand(channels, 1, -1, -1)
            )
...
class HighPassTwoBasisP5Fusion(nn.Module):
    """Fuse low-frequency semantics and initialized high-frequency detail."""
    ...
    def __init__(
        ...
        self.downsampling = nn.ModuleList(
            [
                HighPassTwoBasisDownsample(channels, anti_alias=anti_alias)
...
class ScaleDecoupledReassemblyProjectorV13(nn.Module):
    """Keep exact P3/P4 and use a frequency-aware two-basis P5 branch."""
    ...
    def __init__(
        ...
        if unsupported:
            raise ValueError(
                f"SDSR-v13 Projector does not support levels: {sorted(unsupported)}"
            )
        if not levels:
        ...
        if "P5" in self.levels:
            self.branches["P5"] = HighPassTwoBasisP5Fusion(
                in_channels,
                out_channels,
                num_blocks=num_blocks,
