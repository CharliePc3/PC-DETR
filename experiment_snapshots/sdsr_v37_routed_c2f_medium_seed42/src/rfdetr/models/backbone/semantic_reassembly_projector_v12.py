...
"""Scale-decoupled projector with a four-basis P5 phase representation."""
...
class FourBasisDownsample(nn.Module):
    """Expose one low-pass and three learned spatial responses per channel."""
    ...
    def __init__(self, channels: int, anti_alias: bool):
        ...
        self.detail_norm = LayerNorm(3 * channels)
        self.detail_scale = nn.Parameter(
            torch.full((1, 3 * channels, 1, 1), 0.1)
        )
...
class FourBasisP5Fusion(nn.Module):
    """Preserve four spatial bases until global semantic fusion."""
    ...
    def __init__(
        ...
        self.downsampling = nn.ModuleList(
            [
                FourBasisDownsample(channels, anti_alias=anti_alias)
...
class ScaleDecoupledReassemblyProjectorV12(nn.Module):
    """Retain exact P3/P4 fusion and four spatial bases for P5."""
    ...
    def __init__(
        ...
        if unsupported:
            raise ValueError(
                f"SDSR-v12 Projector does not support levels: {sorted(unsupported)}"
            )
        if not levels:
        ...
        if "P5" in self.levels:
            self.branches["P5"] = FourBasisP5Fusion(
                in_channels,
                out_channels,
                num_blocks=num_blocks,
