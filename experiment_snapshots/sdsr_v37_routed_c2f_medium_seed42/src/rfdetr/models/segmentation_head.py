...
class SegmentationHead(nn.Module):
    def __init__(
        ...
        # Keep P3 as the spatial anchor and optionally inject coarser semantic
        # context.  The per-channel gates start at zero, so enabling this path
        # is exactly equivalent to the original single-level head at init.
        num_context_levels = num_feature_levels - 1
        if num_context_levels > 0:
            num_groups = min(32, in_dim)
            while in_dim % num_groups != 0:
                num_groups -= 1
            self.context_projs = nn.ModuleList(
                [
                    ...
                    for _ in range(num_context_levels)
                ]
            )
            self.context_gates = nn.Parameter(torch.zeros(num_context_levels, in_dim))
        else:
            self.context_projs = nn.ModuleList()
            self.register_parameter("context_gates", None)

        self._export = False

    def _fuse_spatial_features(
        self,
        spatial_features: torch.Tensor | list[torch.Tensor],
        target_size: tuple[int, int],
    ) -> torch.Tensor:
        if isinstance(spatial_features, torch.Tensor):
            feature_levels = [spatial_features]
        else:
            feature_levels = list(spatial_features)
        if len(feature_levels) != self.num_feature_levels:
            raise ValueError(
                f"Expected {self.num_feature_levels} mask feature levels, "
                f"but received {len(feature_levels)}."
            )

        fused = F.interpolate(feature_levels[0], size=target_size, mode="bilinear", align_corners=False)
        for level_index, (context, projection) in enumerate(
            zip(feature_levels[1:], self.context_projs)
        ):
            context = F.interpolate(context, size=target_size, mode="bilinear", align_corners=False)
            gate = self.context_gates[level_index].view(1, -1, 1, 1)
            fused = fused + gate * projection(context)
        return fused
    ...
    def forward(
        ...
        spatial_features = self._fuse_spatial_features(spatial_features, target_size)
...
def get_target_boundary_point_coords(
    target_masks: torch.Tensor,
    num_points: int,
) -> torch.Tensor:
    """Sample normalized pixel centers from one-pixel target-mask boundaries.

    ``target_masks`` must have shape ``[N, 1, H, W]``. Empty or constant
    masks fall back to uniform pixel sampling so every matched instance still
    contributes the requested number of points.
    """

    if target_masks.ndim != 4 or target_masks.shape[1] != 1:
        raise ValueError("target_masks must have shape [N, 1, H, W]")
    if num_points < 0:
        raise ValueError("num_points must be non-negative")
    num_masks, _, height, width = target_masks.shape
    if num_points == 0:
        return target_masks.new_empty((num_masks, 0, 2))

    masks = (target_masks > 0.5).float()
    ...
    weights = (dilated - eroded).clamp_min(0).flatten(1)
    empty = weights.sum(dim=1) == 0
    if empty.any():
        weights[empty] = 1.0
    indices = torch.multinomial(weights, num_points, replacement=True)

    x = (indices.remainder(width).float() + 0.5) / width
    y = (indices.div(width, rounding_mode="floor").float() + 0.5) / height
    return torch.stack((x, y), dim=-1).to(dtype=target_masks.dtype)
