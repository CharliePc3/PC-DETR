...
def make_output_dir(args):
    ...
    if args.projector_type in {
        ...
        projector_tag = {
            ...
            "sdsr_v35": "sdsrv35",
            "sdsr_v36": "sdsrv36",
            "sdsr_v37_r1": "sdsrv37r1",
            "sdsr_v37_r2": "sdsrv37r2",
            "sdsr_v37_r3": "sdsrv37r3",
            "sdsr_v38_p3": "sdsrv38p3",
...
def parse_args():
    ...
    parser.add_argument(
        "--matcher-quality-mode",
        default="none",
        choices=("none", "aux_group_gate", "cls_ignore", "aux_group_gate_cls_ignore"),
        help="Quality control applied after Hungarian matching.",
    )
    parser.add_argument("--matcher-quality-start-epoch", type=int, default=2)
    parser.add_argument("--matcher-quality-ramp-epoch", type=int, default=8)
    parser.add_argument(
        "--matcher-quality-thresholds",
        type=float,
        nargs=3,
        default=[0.1, 0.2, 0.3],
        metavar=("SMALL", "MEDIUM", "LARGE"),
    )
    parser.add_argument("--matcher-quality-log", action="store_true")
...
def main():
    ...
    if args.matcher_quality_start_epoch < 0:
        raise ValueError("Matcher quality start epoch must be non-negative.")
    if args.matcher_quality_ramp_epoch < args.matcher_quality_start_epoch:
        raise ValueError("Matcher quality ramp epoch must be >= its start epoch.")
    if any(not 0 <= threshold <= 1 for threshold in args.matcher_quality_thresholds):
        raise ValueError("Matcher quality thresholds must lie in [0, 1].")
    if args.use_dense_o2o:
        if args.use_budgeted_sa:
            raise ValueError(
    ...
    log_main(f"matcher_quality_mode={args.matcher_quality_mode}")
    log_main(
        "matcher_quality_schedule="
        f"start={args.matcher_quality_start_epoch}, "
        f"ramp={args.matcher_quality_ramp_epoch}, "
        f"thresholds={args.matcher_quality_thresholds}"
    )
    log_main(f"matcher_quality_log={args.matcher_quality_log}")
    ...
    model.train(
        ...
        matcher_quality_mode=args.matcher_quality_mode,
        matcher_quality_start_epoch=args.matcher_quality_start_epoch,
        matcher_quality_ramp_epoch=args.matcher_quality_ramp_epoch,
        matcher_quality_thresholds=tuple(args.matcher_quality_thresholds),
        matcher_quality_log=args.matcher_quality_log,
