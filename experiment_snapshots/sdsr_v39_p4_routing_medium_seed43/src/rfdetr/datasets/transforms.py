...
class AlbumentationsWrapper:
    ...
    def _clip_and_filter_boxes(
        boxes: np.ndarray, width: int, height: int, eps: float = 1e-3
    ) -> Tuple[np.ndarray, np.ndarray]:
        """Clip Pascal VOC boxes to image bounds and return a valid-box mask."""
        ...
        boxes[:, [1, 3]] = np.clip(boxes[:, [1, 3]], 0, float(height))
        valid = (
    ...
    def _apply_geometric_transform(
        ...
        boxes_np, valid = self._clip_and_filter_boxes(boxes_np, width, height)
        if valid.shape[0] > 0 and not bool(valid.all()):
            boxes_np = boxes_np[valid]
            labels = [label for label, keep in zip(labels, valid) if keep]
            idxs = [idx for idx, keep in zip(idxs, valid) if keep]
            if masks_list is not None:
                masks_list = [mask for mask, keep in zip(masks_list, valid) if keep]
        ...
        kept_idxs = list(augmented.get("idxs", idxs))
        aug_height, aug_width = augmented["image"].shape[:2]
        bboxes_aug, valid_aug = self._clip_and_filter_boxes(bboxes_aug, aug_width, aug_height)
        if valid_aug.shape[0] > 0 and not bool(valid_aug.all()):
            bboxes_aug = bboxes_aug[valid_aug]
            kept_idxs = [idx for idx, keep in zip(kept_idxs, valid_aug) if keep]
            augmented["category_ids"] = [
                label for label, keep in zip(augmented["category_ids"], valid_aug) if keep
            ]
