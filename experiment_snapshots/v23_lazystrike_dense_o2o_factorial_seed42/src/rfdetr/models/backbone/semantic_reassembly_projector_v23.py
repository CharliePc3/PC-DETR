...
"""Depth-adaptive grouped detail sampling for efficient P5 reassembly."""
...
class DepthAdaptiveGroupedDetailP5Fusion(nn.Module):
    """Open cross-channel detail sampling only for the deepest ViT feature."""
...
class ScaleDecoupledReassemblyProjectorV23(nn.Module):
    """Preserve v11 while adding semantic detail mixing at the deepest layer."""
    ...
    def __init__(
        ...
        if unsupported:
            raise ValueError(
                f"SDSR-v23 Projector does not support levels: {sorted(unsupported)}"
            )
        if not levels:
