...
QUEUE_LOG="${ROOT}/unified_refinement_phase_b_gpu0_queue_nohup.log"
PID_FILE="${ROOT}/unified_refinement_phase_b_gpu0_queue.pid"
LOCK_FILE="${ROOT}/unified_refinement_phase_b_gpu0_queue.lock"
SUMMARY_DIR="${ROOT}/output/unified_refinement"
SUMMARY_TSV="${SUMMARY_DIR}/phase_b_summary.tsv"

cd "${ROOT}"
mkdir -p "${SUMMARY_DIR}" /tmp/matplotlib_unified_phase_b
export CUDA_VISIBLE_DEVICES="${GPU_ID}"
export PYTHONPATH="${ROOT}"
export MPLCONFIGDIR="/tmp/matplotlib_unified_phase_b"
export ALBUMENTATIONS_DISABLE_VERSION_CHECK=1

exec 9>"${LOCK_FILE}"
if ! flock -n 9; then
  printf '[%s] Another unified Phase-B queue holds %s\n' "$(date '+%F %T')" "${LOCK_FILE}" >&2
  exit 1
fi
printf '%s\n' "$$" > "${PID_FILE}"
...
run_refine() {
  ...
  if [[ -s "${refine_dir}/checkpoints/model_epoch2.pt" ]]; then
    log_status "SKIP refinement ${variant}: epoch-2 checkpoint exists"
    return
  fi
  wait_for_gpu
...
run_diagnostic() {
  variant="$1"
  refine_name="dinov3_small_coco5k_unified_${variant}_e2"
  checkpoint="${ROOT}/output/unified_refinement/${refine_name}/checkpoints/model_epoch2.pt"
...
run_detection() {
  ...
  refine_name="dinov3_small_coco5k_unified_${variant}_e2"
  checkpoint="${ROOT}/output/unified_refinement/${refine_name}/checkpoints/model_epoch2.pt"
  detect_name="coco_medium_tb16_bs8_g6_p3p4p5_ms_exp_cdn_dn50_box0.6_loss0.5_unified_${variant}_e2_e24"
  detect_dir="${ROOT}/output/${detect_name}"
  detect_log="${ROOT}/medium_tb16_bs8_g6_p3p4p5_ms_exp_cdn_dn50_box0.6_loss0.5_unified_${variant}_e2_e24_nohup.log"
...
record_result() {
  variant="$1"
  mode="$2"
  detect_dir="${ROOT}/output/coco_medium_tb16_bs8_g6_p3p4p5_ms_exp_cdn_dn50_box0.6_loss0.5_unified_${variant}_e2_e24"
  diag_dir="${ROOT}/output/token_analysis/unified_${variant}_cocoval200"
  if [[ ! -s "${SUMMARY_TSV}" ]]; then
    printf 'variant\tmode\tbest_epoch\tAP\tAP50\tAP75\tAPs\tAPm\tAPl\tlast5_AP\tcoverage100\tsmall_coverage100\tmedium_coverage100\tlarge_coverage100\tcls_hflip_cosine\tfp_gp_proxy_ratio\n' > "${SUMMARY_TSV}"
  fi
...
run_variant "uni_balanced_to_lazy_v2_seq" "sequential"
run_variant "uni_balanced_lazy_v2_joint_u005" "joint"
