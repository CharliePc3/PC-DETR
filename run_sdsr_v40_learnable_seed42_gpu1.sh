...
EXPERIMENT="sdsr_v40_learnable_medium_seed42"
...
RUN_NAME="coco_medium_s42_detinit1042_sdsr_v40_p4_learnable_strong_lrd20_e24"
...
exec >> "${MASTER_LOG}" 2>&1
printf '[%s] START gpu=%s used=%sMiB snapshot=%s\n' \
  "$(date '+%F %T')" "${GPU_ID}" "${used_mib}" "${SNAPSHOT_ROOT}"
