...
FIRST_RESULTS=/data/cpc/root/project/RF-DETR-DINOv3/output/coco_medium_tb16_bs8_g6_p3p4p5_ms_exp_cdn_dn50_box0.6_loss0.5_lazystrike_v2_align0p15_e24/results.json
SECOND_RESULTS=/data/cpc/root/project/RF-DETR-DINOv3/output/coco_medium_tb16_bs8_g6_p3p4p5_ms_exp_cdn_dn50_box0.6_loss0.5_lazystrike_v1v2_midstrong_e24/results.json

while [[ ! -f "$FIRST_RESULTS" ]]; do
  sleep 300
done

if [[ -f "$SECOND_RESULTS" ]]; then
  echo "Second run already has results: $SECOND_RESULTS"
  exit 0
fi
