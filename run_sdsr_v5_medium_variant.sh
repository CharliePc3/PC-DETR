...
VARIANT="${VARIANT:?Set VARIANT to guided, semantic, or guided_antialias}"
MAX_USED_MIB="${MAX_USED_MIB:-4000}"
POLL_SECONDS="${POLL_SECONDS:-60}"

case "${VARIANT}" in
  guided)
    OUTPUT_TAG="jointfield_guided_rank64"
    EXTRA_ARGS=(
    ...
    ;;
  semantic)
    OUTPUT_TAG="jointfield_semantic_rank64"
    EXTRA_ARGS=(
      --sdsr-use-local-reassembly
    ...
    ;;
  guided_antialias)
    OUTPUT_TAG="jointfield_guided_antialias_rank64"
    EXTRA_ARGS=(
      --sdsr-use-local-reassembly
      --sdsr-use-directional-guide
...
OUTPUT_NAME="coco_medium_tb16_bs8_g6_p3p4p5_ms_exp_cdn_sdsrv5_${OUTPUT_TAG}_seed42_e24"
OUTPUT_DIR="${ROOT}/output/${OUTPUT_NAME}"
TRAIN_LOG="${ROOT}/${OUTPUT_NAME}_nohup.log"
RUN_LOG="${ROOT}/sdsr_v5_${VARIANT}_gpu${GPU_ID}_runner_nohup.log"
PID_FILE="${ROOT}/sdsr_v5_${VARIANT}_gpu${GPU_ID}.pid"
LOCK_FILE="${ROOT}/sdsr_v5_${VARIANT}_gpu${GPU_ID}.lock"
RESULT_DIR="${ROOT}/output/sdsr_v5_medium_ablation"
RESULT_TSV="${RESULT_DIR}/${VARIANT}_seed42.tsv"

cd "${ROOT}"
mkdir -p "${RESULT_DIR}" "/tmp/matplotlib_sdsr_v5_${VARIANT}_${GPU_ID}"
export CUDA_VISIBLE_DEVICES="${GPU_ID}"
export PYTHONPATH="${ROOT}/src"
export MPLCONFIGDIR="/tmp/matplotlib_sdsr_v5_${VARIANT}_${GPU_ID}"
...
RESUME_ARGS=()
if [[ -s "${OUTPUT_DIR}/checkpoint.pth" ]] && [[ $(wc -l < "${OUTPUT_DIR}/log.txt" 2>/dev/null || echo 0) -lt 24 ]]; then
...
if [[ -s "${OUTPUT_DIR}/log.txt" ]] && [[ $(wc -l < "${OUTPUT_DIR}/log.txt") -ge 24 ]]; then
  ...
  log_status "START ${VARIANT} on physical GPU ${GPU_ID}, output=${OUTPUT_NAME}"
