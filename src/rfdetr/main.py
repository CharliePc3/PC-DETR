...
def _log_detailed_metrics(tag: str, stats: dict) -> None:
    ...
    class_map = results_json.get("class_map", []) if isinstance(results_json, dict) else []
    if not class_map:
        return
    ...
    def train(self, callbacks: DefaultDict[str, List[Callable]], **kwargs):
        ...
        if is_main_process():
            ...
            if args.use_ema:
                ...
                best_source = (
                    "ema"
                    if best_summary.get("ema_best_res", float("-inf")) >= best_summary.get("regular_best_res", float("-inf"))
                    else "regular"
                )
            else:
...
def get_args_parser():
    ...
    # * Backbone
    parser.add_argument("--encoder", default="dinov3_small", type=str, help="Name of the DINOv3 encoder to use")
