...
"""Mean-centered spatial P4 residual routing on top of SDSR-v40."""
...
def _valid_spatial_mean(
    values: torch.Tensor,
    padding_mask: torch.Tensor | None,
) -> torch.Tensor:
    if padding_mask is None:
        return values.mean(dim=(-2, -1), keepdim=True)
    valid = (~padding_mask).to(values.dtype)
    denominator = valid.sum(dim=(-2, -1), keepdim=True).clamp_min(1)
    return (values * valid).sum(dim=(-2, -1), keepdim=True) / denominator


class MeanCenteredSpatialDepthResidual(nn.Module):
    """Predict a bounded, zero-mean depth correction at each P4 location."""
    ...
    def forward(
        ...
        logits = logits - _valid_spatial_mean(logits, resized_mask)

        bounded = torch.tanh(logits)
        bounded = bounded - _valid_spatial_mean(bounded, resized_mask)
        max_abs = bounded.abs()
        if resized_mask is not None:
            max_abs = max_abs.masked_fill(resized_mask, 0)
        scale = max_abs.amax(dim=(-2, -1), keepdim=True).clamp_min(1)
        residual = self.max_logit_delta * bounded / scale
        if resized_mask is not None:
            residual = residual.masked_fill(resized_mask, 0)
        return residual


class MeanCenteredSpatialResidualP4Fusion(nn.Module):
    """Add only local depth variation to the proven v40 global P4 prior."""
    ...
    def __init__(
        ...
        self.base = base
        self.spatial_residual = MeanCenteredSpatialDepthResidual(
            channels=channels,
...
class ScaleDecoupledReassemblyProjectorV45SpatialCentered020(
    ScaleDecoupledReassemblyProjectorV40P4Learnable
):
    """Keep the v40 global prior and add a zero-mean local P4 correction."""

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if "P4" not in self.branches:
            raise ValueError("v45 spatial routing requires a P4 output")
        in_channels = tuple(args[0] if args else kwargs["in_channels"])
        if not in_channels or len(set(in_channels)) != 1:
            raise ValueError("v45 expects equal-channel ViT features")
        rank_channels = max(8, int(kwargs.get("rank_channels", 64)) // 8)

        rng_state = torch.get_rng_state()
        try:
            self.branches["P4"] = MeanCenteredSpatialResidualP4Fusion(
                self.branches["P4"],
