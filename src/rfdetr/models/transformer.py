...
class Transformer(nn.Module):
    def __init__(
        ...
        bbox_reparam=False,
        query_init="learned",
        query_memory_detach=True,
        query_init_gate=0.0,
        scale_routing=False,
        scale_routing_mode="legacy",
        scale_routing_layers=None,
        p5_attention_bias=0.0,
    ):
        super().__init__()
        self.encoder = None
        if scale_routing_layers is not None and not scale_routing:
            raise ValueError("scale_routing_layers require scale_routing=True.")
        ...
        decoder_layer = TransformerDecoderLayer(
            ...
            last_level_initial_bias=p5_attention_bias,
        )
        ...
        self.decoder = TransformerDecoder(
            ...
            scale_routing_layers=scale_routing_layers,
        )
        ...
        if query_init not in {"learned", "hybrid_topk"}:
            raise ValueError(f"Unsupported query_init: {query_init}")
        if query_init == "hybrid_topk" and not two_stage:
            raise ValueError("hybrid_topk query initialization requires two_stage=True")
        self.query_init = query_init
        self.query_memory_detach = query_memory_detach
        if query_init == "hybrid_topk":
            self.query_memory_proj = nn.Sequential(
                nn.Linear(d_model, d_model),
                nn.LayerNorm(d_model),
            )
            self.query_memory_gate = nn.Parameter(torch.tensor(float(query_init_gate)))
    ...
    def _reset_parameters(self):
        scale_router_params = {
            ...
            if isinstance(module, MSDeformAttn) and module.scale_router is not None
            for parameter in module.scale_router.parameters()
        }
        for p in self.parameters():
            if id(p) not in scale_router_params and p.dim() > 1:
    ...
    def forward(
        ...
        if self.dec_layers > 0:
            ...
            if self.two_stage:
                ...
                if self.query_init == "hybrid_topk":
                    proposal_content = memory_ts.detach() if self.query_memory_detach else memory_ts
                    proposal_content = self.query_memory_proj(proposal_content)
                    tgt = torch.cat(
                        [
                            tgt[..., :ts_len, :]
                            + torch.tanh(self.query_memory_gate) * proposal_content,
                            tgt[..., ts_len:, :],
                        ],
                        dim=-2,
                    )
...
class TransformerDecoder(nn.Module):
    def __init__(
        ...
        self.bbox_embed_layers = None
        self.bbox_embed_residuals = None
        self.bbox_refine_mode = "shared"
        if scale_routing_layers is not None:
            selected_layers = set(scale_routing_layers)
            if any(layer < 0 or layer >= num_layers for layer in selected_layers):
                raise ValueError(f"scale_routing_layers must be in [0, {num_layers - 1}].")
            for layer_id, layer in enumerate(self.layers):
                if layer_id not in selected_layers:
                    layer.cross_attn.disable_scale_routing()
    ...
    def configure_bbox_refinement(self, bbox_embed, mode="shared"):
        if mode not in {"shared", "layerwise", "residual"}:
            raise ValueError(f"Unsupported bbox_refine_mode: {mode}")

        self.bbox_embed = bbox_embed
        self.bbox_refine_mode = mode
        if mode == "layerwise":
            self.bbox_embed_layers = nn.ModuleList(
                [bbox_embed] + [copy.deepcopy(bbox_embed) for _ in range(self.num_layers - 1)]
            )
        elif mode == "residual":
            # Keep the shared prediction intact at initialization, then let each
            # decoder layer learn a small content-dependent correction.
            with torch.random.fork_rng(devices=[]):
                self.bbox_embed_residuals = nn.ModuleList(
                    [nn.Linear(self.d_model, 4) for _ in range(self.num_layers)]
                )
            for residual in self.bbox_embed_residuals:
                nn.init.constant_(residual.weight, 0.0)
                nn.init.constant_(residual.bias, 0.0)

    def bbox_delta(self, output, layer_id):
        if self.bbox_embed is None:
            raise RuntimeError("Iterative bbox refinement is not configured.")
        if self.bbox_refine_mode == "layerwise":
            return self.bbox_embed_layers[layer_id](output)

        delta = self.bbox_embed(output)
        if self.bbox_refine_mode == "residual":
            delta = delta + self.bbox_embed_residuals[layer_id](output)
        return delta

    def bbox_deltas(self, hidden_states):
        if hidden_states.shape[0] != self.num_layers:
            raise ValueError(
                f"Expected {self.num_layers} decoder hidden states, got {hidden_states.shape[0]}."
            )
        return torch.stack(
            [self.bbox_delta(hidden_states[layer_id], layer_id) for layer_id in range(self.num_layers)]
        )
    ...
    def forward(
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if not self.lite_refpoint_refine:
                ...
                new_refpoints_delta = self.bbox_delta(output, layer_id)
...
class TransformerDecoderLayer(nn.Module):
    def __init__(
        ...
        self.cross_attn = MSDeformAttn(
            ...
            scale_routing=scale_routing,
            scale_routing_mode=scale_routing_mode,
            last_level_initial_bias=last_level_initial_bias,
        )
...
def build_transformer(args):
    ...
    return Transformer(
        ...
        query_init=getattr(args, "query_init", "learned"),
        query_memory_detach=getattr(args, "query_memory_detach", True),
        query_init_gate=getattr(args, "query_init_gate", 0.0),
        scale_routing=getattr(args, "scale_routing", False),
        scale_routing_mode=getattr(args, "scale_routing_mode", "legacy"),
        scale_routing_layers=getattr(args, "scale_routing_layers", None),
        p5_attention_bias=getattr(args, "p5_attention_bias", 0.0),
    )
