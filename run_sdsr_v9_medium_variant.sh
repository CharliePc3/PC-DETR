...
VARIANT="${VARIANT:?Set VARIANT to anchor, anchor_rank96, anchor_bidir, or anchor_noaa}"
...
case "${VARIANT}" in
  anchor)
    OUTPUT_TAG="anchor_rank64_stableinit${DETECTOR_INIT_SEED}"
    PROJECTOR_ARGS=(
      --projector-type sdsr_v9
    ...
    ;;
  anchor_rank96)
    OUTPUT_TAG="anchor_rank96_stableinit${DETECTOR_INIT_SEED}"
    PROJECTOR_ARGS=(
      --projector-type sdsr_v9
    ...
    ;;
  anchor_bidir)
    OUTPUT_TAG="anchor_bidir32_rank64_stableinit${DETECTOR_INIT_SEED}"
    PROJECTOR_ARGS=(
      --projector-type sdsr_v9
      --sdsr-rank-channels 64
    ...
    ;;
  anchor_noaa)
    OUTPUT_TAG="anchor_noaa_rank64_stableinit${DETECTOR_INIT_SEED}"
    PROJECTOR_ARGS=(
      --projector-type sdsr_v9
      --sdsr-rank-channels 64
      --sdsr-cross-scale-mode none
...
OUTPUT_NAME="coco_medium_tb16_bs8_g6_p3p4p5_ms_exp_cdn_sdsrv9_${OUTPUT_TAG}_seed42_e24"
OUTPUT_DIR="${ROOT}/output/${OUTPUT_NAME}"
TRAIN_LOG="${ROOT}/${OUTPUT_NAME}_nohup.log"
RUN_LOG="${ROOT}/sdsr_v9_${VARIANT}_gpu${GPU_ID}_runner_nohup.log"
PID_FILE="${ROOT}/sdsr_v9_${VARIANT}_gpu${GPU_ID}.pid"
LOCK_FILE="${ROOT}/sdsr_v9_${VARIANT}_gpu${GPU_ID}.lock"
RESULT_DIR="${ROOT}/output/sdsr_v9_medium_ablation"
RESULT_TSV="${RESULT_DIR}/${VARIANT}_seed42.tsv"

cd "${ROOT}"
mkdir -p "${RESULT_DIR}" "/tmp/matplotlib_sdsr_v9_${VARIANT}_${GPU_ID}"
export CUDA_VISIBLE_DEVICES="${GPU_ID}"
export PYTHONPATH="${ROOT}/src"
export MPLCONFIGDIR="/tmp/matplotlib_sdsr_v9_${VARIANT}_${GPU_ID}"
