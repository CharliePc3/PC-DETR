...
EXPERIMENT="sdsr_v38_adaptive_c2f_medium_seed42"
...
# Freeze source before waiting so concurrent sessions cannot alter queued runs.
...
wait_for_gpu() {
  ...
  while true; do
    ...
    if [[ "${used_mib}" =~ ^[0-9]+$ ]] \
        && (( used_mib <= MAX_USED_MIB )); then
      printf '[%s] GPU_READY gpu=%s used=%sMiB threshold=%sMiB\n' \
        "$(date '+%F %T')" "${GPU_ID}" "${used_mib}" "${MAX_USED_MIB}"
      return
    fi
    printf '[%s] WAIT gpu=%s used=%sMiB threshold=%sMiB\n' \
...
run_cell() {
  ...
  local description="$3"
  local run_name="coco_medium_s42_detinit1042_${cell}_lrd20_e24"
...
run_cell \
  "sdsr_v23_currentcontrol" \
  "sdsr_v23" \
  "Same-snapshot v23 control for strict attribution"
run_cell \
  "sdsr_v38_p3_localres_c2f" \
  "sdsr_v38_p3" \
  "Low-rank local-attention P3 residual while retaining ConvTranspose and C2f"
run_cell \
  "sdsr_v38_p4_dynamicroute_c2f" \
  "sdsr_v38_p4" \
  "Per-image normalized ViT-layer routing while retaining the P4 C2f"
