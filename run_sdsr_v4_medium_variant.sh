...
case "${VARIANT}" in
  guided)
    OUTPUT_TAG="layerwise_guided_rank64"
    EXTRA_ARGS=(
    ...
    ;;
  semantic)
    OUTPUT_TAG="layerwise_semantic_rank64"
    EXTRA_ARGS=(
      --sdsr-use-local-reassembly
    ...
    ;;
  guided_antialias)
    OUTPUT_TAG="layerwise_guided_antialias_rank64"
    EXTRA_ARGS=(
      --sdsr-use-local-reassembly
      --sdsr-use-directional-guide
...
OUTPUT_NAME="coco_medium_tb16_bs8_g6_p3p4p5_ms_exp_cdn_sdsrv4_${OUTPUT_TAG}_seed42_e24"
OUTPUT_DIR="${ROOT}/output/${OUTPUT_NAME}"
TRAIN_LOG="${ROOT}/${OUTPUT_NAME}_nohup.log"
RUN_LOG="${ROOT}/sdsr_v4_${VARIANT}_gpu${GPU_ID}_runner_nohup.log"
PID_FILE="${ROOT}/sdsr_v4_${VARIANT}_gpu${GPU_ID}.pid"
LOCK_FILE="${ROOT}/sdsr_v4_${VARIANT}_gpu${GPU_ID}.lock"
RESULT_DIR="${ROOT}/output/sdsr_v4_medium_ablation"
RESULT_TSV="${RESULT_DIR}/${VARIANT}_seed42.tsv"

cd "${ROOT}"
mkdir -p "${RESULT_DIR}" "/tmp/matplotlib_sdsr_v4_${VARIANT}_${GPU_ID}"
export CUDA_VISIBLE_DEVICES="${GPU_ID}"
export PYTHONPATH="${ROOT}/src"
export MPLCONFIGDIR="/tmp/matplotlib_sdsr_v4_${VARIANT}_${GPU_ID}"
