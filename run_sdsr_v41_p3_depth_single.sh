...
VARIANT="${VARIANT:?Set VARIANT to uniform or shallow}"
MAX_USED_MIB="${MAX_USED_MIB:-4000}"
POLL_SECONDS="${POLL_SECONDS:-60}"
SNAPSHOT_ROOT="${ROOT}/experiment_snapshots/sdsr_v41_p3_depth_medium_seed43"

case "${VARIANT}" in
  uniform)
    PROJECTOR_TYPE="sdsr_v41_p3_uniform"
    CELL="sdsr_v41_p3_uniform_p4strong"
    ;;
  shallow)
    PROJECTOR_TYPE="sdsr_v41_p3_shallow"
    CELL="sdsr_v41_p3_shallow_p4strong"
    ;;
  *)
...
EXPERIMENT="${CELL}_medium_seed43"
RUN_NAME="coco_medium_s43_detinit1043_${CELL}_lrd20_e24"
...
if [[ ! -f "${SNAPSHOT_ROOT}/.snapshot_complete" ]]; then
  printf 'Missing frozen v41 snapshot: %s\n' "${SNAPSHOT_ROOT}" >&2
  exit 3
fi
...
exec >> "${QUEUE_LOG}" 2>&1
printf '[%s] START variant=%s projector=%s gpu=%s used=%sMiB snapshot=%s\n' \
  "$(date '+%F %T')" "${VARIANT}" "${PROJECTOR_TYPE}" "${GPU_ID}" \
  "${used_mib}" "${SNAPSHOT_ROOT}"
